...
import com.sunjet.springbookzk.utils.common.UUIDUtil;
...
//    public static String saveCsv(Media media, String path) {
//        String fileName = "";
//        try {
...
////            IOUtils.write(stringData, outputStream,"GB2312");
////            outputStream.close();
//
////            file.createNewFile();
...
//
//
////            EncodingDetect.getJavaEncodeByte(media.getStringData().getBytes());
////            System.out.println(CSVUtil.getEncoding(media.getStringData()));
////            System.out.println(EncodingDetect.getJavaEncodeByte(media.getStringData().getBytes()));
//
////            DataInputStream in = new DataInputStream(new ByteInputStream(media.getStringData()));
////
////            DataOutputStream out = new DataOutputStream(new FileOutputStream(file));
////            OutputStreamWriter writer = new OutputStreamWriter(out,EncodingDetect.getJavaEncodeByte(media.getByteData()));
////            writer.write
//            System.out.println(CSVUtil.getEncoding(media.getFormat()));
//            FileUtils.write(file,media.getStringData(),CSVUtil.getEncoding(media.getFormat()));
////            Files.copy(file,IOUtils.toInputStream(stringData));
////            Files.copy(file,media.getReaderData() ,fileEncode);
//
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
...
//            InputStream fin = media.getStringData();
...
////		boolean isOk = false;
////		if (!StringUtils.isEmptyString(vm.getMenuEntity().get("menu_id") + "")) {
////			String key = menuOperation.name();
////			String mpString = ((Map)(vm.getMenuEntity().get("pms"))).get(key)+ "";
////
...
//            Messagebox.show(message, title, Messagebox.OK, "z-icon-info-circle");
        } catch (Exception e) {
            // ignore
        }
    }
...
//            Messagebox.show(message, title, Messagebox.OK, "z-icon-exclamation-triangle");
        } catch (Exception e) {
            // ignore
        }
    }
...
//            Messagebox.show(message, title, Messagebox.OK, "z-icon-times-circle");
        } catch (Exception e) {
            // ignore
        }
    }
...
//            return Messagebox.OK == Messagebox.show(message, title, Messagebox.OK | Messagebox.CANCEL, "z-icon-question-circle-o");
        } catch (Exception e) {
            // ignore
            return false;
        }
...
//            Messagebox.show(message, title, Messagebox.OK | Messagebox.CANCEL, "z-icon-question-circle-o", eventListener);
        } catch (Exception e) {
            // ignore
        }
        ...
        for (Field field : fields) {
            ...
            if (field.isAccessible()) {
                ...
                if (!ignoreFields.contains(field.getName())) {
                    try {
                        if (field.get(entity) == null || StringUtils.isEmpty(field.get(entity).toString())) {
